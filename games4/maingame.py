...
img_boss = load_img("lastboss.png", (240, 240), RD)

# 1. Load Gambar Side Boss (Sebagai Musuh)
...
# 2. Load Gambar Companion (Sebagai Teman)
sz_ally = (int(H*0.06), int(H*0.06)) # Ukuran teman lebih kecil
img_ally_hutan = load_img("ally_hutan.png", sz_ally, PU)
img_ally_gurun = load_img("ally_gurun.png", sz_ally, YL)
img_ally_rawa  = load_img("ally_rawa.png", sz_ally, (139, 69, 19))
img_ally_salju = load_img("ally_salju.png", sz_ally, CY)
...
img_m_normal = load_img("monster_hijau.png", (sz_m, sz_m), YL)  # Monster biasa
img_m_tank   = load_img("monster_biru.png", (sz_m, sz_m), BL)   # Monster kamikaze (Bom)
img_m_fast   = load_img("monster_merah.png", (sz_m, sz_m), RD)  # Monster cepat
...
# ==========================================
# OOP 2: INHERITANCE & ENCAPSULATION
# ==========================================
...
class Companion(Entitas):
    def __init__(self, type_id):
        # type_id mewakili node_id map yang sudah diselesaikan (0=Hutan, 1=Gurun, dst)
...
class Main:
    ...
    def generate_map_objects(self, node_id):
        ...
        try:
            ...
            # === SISTEM TILE ACAK PER MAP ===
            tiles = []
            
            try:
                # KODE YANG DIUBAH: Sekarang dia akan mencari nama file berdasarkan node_id
                ...
                # FALLBACK: Jika gambar tile khusus map ini belum ada, 
                # game akan otomatis membuat variasi warna dari bg_color map tersebut.
                for _ in range(4):
            ...
            # Tempelkan tile secara acak menutupi seluruh map
    ...
    def update(self):
        ...
        if self.state == "STORY": self.dlg.update()
            ...
            if self.state == "HUNT":
                ...
                if self.boss.hp <= 0: 
                    self.score += 10000; self.set_story("boss_win")
                if self.player.get_hp() <= 0: self.set_story("boss_lose")
