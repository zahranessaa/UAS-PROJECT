...
class Player(Entitas):
    def __init__(self, x, y):
        ...
        self.__hp = 100       # Private Variable
        self.__max_hp = 100   # Private Variable
        self.hp_regen = 0.04 
...
class Monster(Entitas):
    ...
    def update(self, *args, **kwargs): pass # Memenuhi aturan Abstraksi
...
class Main:
    ...
    def save_game(self):
        data = {
            ...
            "ammo": self.ammo, "level": self.player.level, "max_hp": self.player.get_max_hp(),
        ...
        if self.state == "STORY": self.dlg.update()
            ...
            for p in self.peluru[:]:
                ...
                if p.is_player:
                    ...
                    if math.hypot(p.x - self.player.x, p.y - self.player.y) < H*0.03 and self.player.invuln <= 0:
                        self.player.take_damage(15); self.player.invuln = 45; self.peluru.remove(p); play_tone(100, 100)
                if p.x < 0 or p.x > self.world_w or p.y < 0 or p.y > self.world_h:
                    if p in self.peluru: self.peluru.remove(p)

            if dist_boss < H * 0.15 and self.player.invuln <= 0:
                self.player.take_damage(20); self.player.invuln = 45; play_tone(100, 100)
    ...
    def draw(self):
        if self.state == "TITLE":
            ...
            txt(scr, f"Lv {self.player.level}", F18, WH, 20, 10)
            ...
            txt(scr, f"Listrik (1): {stat_cd}", F14, WH, 20, 85)
