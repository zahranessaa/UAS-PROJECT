...
class Companion(Entitas):
    def __init__(self, type_id):
        super().__init__(0, 0, tint_image(img_ally, [YL, OR, (139, 69, 19), CY][type_id % 4]))
...
class Main:
    ...
    def generate_map_objects(self, node_id):
        ...
        try:
            map_img = pygame.image.load(os.path.join(BASE_PATH, f"map_{node_id}.png")).convert()
            ...
            if self.state == "TITLE":
                for i in range(len(title_opts)):
                    if pygame.Rect(W//2 - W*0.2, H*0.45 + i*(H*0.08) - H*0.03, W*0.4, H*0.06).collidepoint(mx, my):
                        ...
                        elif act == "Pengaturan": self.sel = 0; self.state = "OPTIONS"
        ...
        if self.state == "STORY": self.dlg.update()
            ...
            if self.state == "HUNT":
                ...
                if self.stage_timer < max_timer:
                    level_mult = 1 + (self.player.level * 0.1) 
                    ...
                    if self.stage_timer % spawn_rate == 0 and len(self.monsters) < max_monsters:
                        ...
                        diff = (1 + (self.stage_timer / max_timer)) * level_mult
                    ...
                    sb = Monster(spawn_x, spawn_y, stage_diff=3 + (self.player.level*0.2))
                    sb.img = img_sideboss; sb.rect = sb.img.get_rect(center=(sb.x, sb.y))
                    sb.max_hp = 500 * (1 + self.current_node_id * 0.5) + (self.player.level*50)
                ...
                for m in self.monsters[:]:
                    ...
                    if m.stun > 0: m.stun -= 1
                        ...
                        if dist < H * 0.05 and self.player.invuln <= 0:
                            ...
                            self.player.take_damage(dmg); self.player.invuln = 45; play_tone(100, 100)

                        if dist < H * 0.8:
                            ang = math.atan2(self.player.y - m.y, self.player.x - m.x)
                            m.x += math.cos(ang) * m.speed; m.y += math.sin(ang) * m.speed
                            # LOGIKA MENGUBAH HADAP WAJAH MONSTER
                            ...
                            m.x += m.vx; m.y += m.vy
                            # LOGIKA HADAP MONSTER SAAT JALAN ACAK
                ...
                for p in self.peluru_hutan[:]:
                    ...
                    if p.is_player:
                        ...
                        if math.hypot(p.x - self.player.x, p.y - self.player.y) < H*0.03 and self.player.invuln <= 0:
                            self.player.take_damage(5); self.player.invuln = 45; self.peluru_hutan.remove(p); play_tone(100, 100)
                ...
                if self.side_boss_spawned and self.side_boss not in self.monsters:
                    if self.target_node_id not in self.cleared_nodes:
                ...
                # LOGIKA MENGUBAH HADAP WAJAH BOSS
